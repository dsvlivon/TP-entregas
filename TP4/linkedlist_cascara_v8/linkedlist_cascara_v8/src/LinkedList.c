...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
    ...
    xLen = ll_len(this);
    if(this!= NULL && nodeIndex>=0 && nodeIndex<= xLen)
    {
        //LinkedList* this=(LinkedList*)malloc(sizeof(LinkedList*));
        newNode = (Node*)malloc(sizeof(Node*));
        if(newNode!=NULL)
        {
            newNode-> pElement = pElement; // asigno pElement d newNode al traido por parametros
            newNode-> pNextNode = NULL; // declaro el pNextNode en NULL x default
            if(nodeIndex== 0) // se tiene q linkear a pFirstNode
            {
                if(xLen>=1)
                {
            ...
            else if(nodeIndex == xLen)
            {
                auxNode = getNode(this, nodeIndex-1); //obtengo el nodo anterior a nodeIndex
...
void* ll_get(LinkedList* this, int index)
{
    ...
    {
        xLen = ll_len(this);
        if(index<=xLen && index>=0)
        {
            auxNode =getNode(this, index);
            if(auxNode!=NULL)
            {
                auxReturn = auxNode->pElement;
            }
        }
    }
    return auxReturn;
...
int ll_remove(LinkedList* this,int index){
    ...
    if(this != NULL)
    {
        ...
        if(index<xLen && index >= 0 && xLen > 0)
        {
            ...
            {
                auxNode = rNode->pNextNode;
                lNode = getNode(this,index-1);
                lNode ->pNextNode = auxNode;
            }
            free(rNode);
            this->size--;
            auxReturn = 0;
